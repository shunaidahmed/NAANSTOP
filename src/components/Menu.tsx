  ...
  return (
    <section id="menu" className="deferred-section mx-auto max-w-7xl px-4 py-32 sm:px-8 md:py-48 lg:px-12">
      <div className="grid gap-8 lg:grid-cols-[0.7fr_1.3fr] lg:items-end">
        <div>
          <p className="editorial-kicker">{lang === "es" ? "La carta completa" : "The full spread"}</p>
        ...
        {selectedItem && (
          <motion.div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedItem(null)}>
            <motion.div role="dialog" aria-modal="true" aria-label={selectedItem.name} className="cafe-card max-h-[90vh] w-full max-w-lg overflow-auto rounded-2xl p-5" initial={{ opacity: 0, y: 18, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.98 }} onClick={(event) => event.stopPropagation()}>
              ...
              <h3 className="mt-2 font-display text-4xl text-white">{getItemName(selectedItem)}</h3>
              <p className="mt-4 text-sm leading-relaxed text-neutral-400">{getItemDesc(selectedItem)}</p>
              <div className="mt-6 flex items-center justify-between gap-4">
                <span className="font-semibold text-brand">{selectedItem.price}</span>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setSelectedItem(null)} className="border border-white/15 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-neutral-300">{lang === "es" ? "Cerrar" : "Close"}</button>
                  <button type="button" onClick={() => { addToCart(selectedItem); setSelectedItem(null); }} className="bg-brand px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white">{lang === "es" ? "Añadir al pedido" : "Add to order"}</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
          ...
          <motion.div className="cart-overlay fixed inset-0 z-[70]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setCartOpen(false)}>
            <motion.aside role="dialog" aria-modal="true" aria-label={lang === "es" ? "Tu pedido" : "Your order"} className="cart-drawer cafe-card absolute right-0 top-0 flex h-full w-full max-w-[27rem] flex-col rounded-none border-y-0 border-r-0 p-5 sm:p-7" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 360, damping: 34 }} onClick={(event) => event.stopPropagation()}>
              <div className="flex shrink-0 items-start justify-between border-b border-white/10 pb-5">
                <div>
                  <p className="editorial-kicker">{lang === "es" ? "Listo para enviar" : "Ready to send"}</p>
                  <h3 className="mt-2 font-display text-3xl text-white">{lang === "es" ? "Tu pedido" : "Your order"}</h3>
                  <p className="mt-1 text-xs text-neutral-500">{cartCount} {cartCount === 1 ? (lang === "es" ? "producto" : "item") : (lang === "es" ? "productos" : "items")}</p>
                </div>
              ...
              <div className="cart-items flex-1 overflow-y-auto py-6">
                {cart.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-white/15 p-6 text-center">
                    <p className="font-display text-xl text-white">{lang === "es" ? "Tu pedido está vacío" : "Your order is empty"}</p>
                    <p className="mt-2 text-sm text-neutral-500">{lang === "es" ? "Añade un plato del menú para empezar." : "Add a dish from the menu to get started."}</p>
                  </div>
                ) : (
                      ...
                      <div key={item.name} className="cart-line rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className="truncate font-display text-lg text-white">{getItemName(item)}</p>
                            <p className="mt-1 text-xs text-brand">{item.price}</p>
                          </div>
                          <button type="button" onClick={() => updateQuantity(item.name, -item.quantity)} aria-label={lang === "es" ? `Eliminar ${getItemName(item)}` : `Remove ${item.name}`} className="text-xs text-neutral-600 transition hover:text-red-300">{lang === "es" ? "Eliminar" : "Remove"}</button>
                        </div>
                        <div className="mt-4 flex items-center justify-between border-t border-white/10 pt-3">
                          <span className="text-xs uppercase tracking-wider text-neutral-500">{lang === "es" ? "Cantidad" : "Quantity"}</span>
              ...
              <div className="shrink-0 border-t border-white/10 pt-5">
                ...
                {cart.length ? (
                  <a href={waLink(`Hi NAAN STOP! I'd like to order:\n${checkoutMessage}`)} target="_blank" rel="noopener noreferrer" onClick={placeOrder} className="inline-flex w-full justify-center rounded-full bg-brand px-6 py-4 text-sm font-semibold uppercase tracking-wider text-white transition hover:bg-brand-dark">{lang === "es" ? "Enviar pedido por WhatsApp" : "Send order on WhatsApp"}</a>
                ) : (
                  <button type="button" onClick={() => setCartOpen(false)} className="inline-flex w-full justify-center rounded-full border border-white/15 px-6 py-4 text-sm font-semibold uppercase tracking-wider text-neutral-500">{lang === "es" ? "Explorar el menú" : "Browse menu"}</button>
                )}
              </div>
            </motion.aside>
          </motion.div>
            ...
            <motion.aside role="dialog" aria-modal="true" aria-label={lang === "es" ? "Historial de pedidos" : "Order history"} className="cart-drawer cafe-card absolute right-0 top-0 flex h-full w-full max-w-[30rem] flex-col rounded-none border-y-0 border-r-0 p-5 sm:p-7" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 360, damping: 34 }} onClick={(event) => event.stopPropagation()}>
              <div className="flex shrink-0 items-start justify-between border-b border-white/10 pb-5">
                <div>
                  ...
                  <h3 className="mt-2 font-display text-3xl text-white">{lang === "es" ? "Historial de pedidos" : "Order history"}</h3>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 border-b border-white/10 py-5 text-center">
                <div><p data-history-count="orders" className="font-display text-2xl text-white">{orderHistory.length}</p><p className="text-[10px] uppercase tracking-wider text-neutral-500">{lang === "es" ? "Pedidos" : "Orders"}</p></div>
                <div><p data-history-count="pending" className="font-display text-2xl text-brand">{pendingOrders.length}</p><p className="text-[10px] uppercase tracking-wider text-neutral-500">{lang === "es" ? "Pendientes" : "Pending"}</p></div>
                <div><p data-history-count="completed" className="font-display text-2xl text-white">{completedOrders.length}</p><p className="text-[10px] uppercase tracking-wider text-neutral-500">{lang === "es" ? "Completados" : "Completed"}</p></div>
              </div>
              ...
              <div className="cart-items flex-1 overflow-y-auto space-y-3">
                {orderHistory.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-white/15 p-6 text-center"><p className="font-display text-xl text-white">{lang === "es" ? "Sin pedidos aún" : "No orders yet"}</p><p className="mt-2 text-sm text-neutral-500">{lang === "es" ? "Tus pedidos enviados aparecerán aquí." : "Your sent orders will appear here."}</p></div>
...
const ItemCard = memo(function ItemCard({ item, index, onDetails, onAdd, lang, getItemName, getItemDesc, getTagLabel }: ItemCardProps) {
  ...
  return (
    <motion.article ref={cardRef} custom={index} variants={variants} initial="hidden" animate={onScreen ? "visible" : "hidden"} whileHover={{ y: -4, scale: 1.008 }} layout className="cafe-card group relative flex h-[31rem] flex-col overflow-hidden rounded-2xl p-5" onClick={() => onDetails(item)}>
      ...
      <div className="flex items-start justify-between gap-3">
        <h3 className="min-h-[3.5rem] font-display text-lg leading-tight tracking-wide text-white">{getItemName(item).toUpperCase()}</h3>
        {item.tag && (
      ...
      <p className="mt-2 flex-1 text-sm leading-relaxed text-neutral-400">{getItemDesc(item)}</p>
      <div className="mt-4 flex items-center justify-between border-t border-neutral-800 pt-4">
        <span className="text-sm font-bold tracking-wide text-brand">{item.price}</span>
        <div className="flex items-center gap-2" onClick={(event) => event.stopPropagation()}>
          <button type="button" onClick={() => onAdd(item)} className="inline-flex h-9 items-center justify-center rounded-full border border-brand/60 px-4 text-[10px] font-semibold uppercase tracking-wider text-brand transition hover:bg-brand hover:text-white">{lang === "es" ? "Añadir" : "Add"}</button>
